...
## Guardian: limit agent autonomy

Businesses need agents to act within customer intent and business policy. Access
to a tool alone does not establish permission for every action. Use this pattern
before consequential writes: assess the proposed action, apply policy, and block
execution when checks fail. Saving customer preferences is the example here.
...
## Semantic Router: match model spend to the work

Routine activities (such as lookups in music store app) and comolex analysis (such as purchase analysis for indivudal customer) have different capability needs. Use this
pattern when recurring requests can be recognised through rules, examples, or
classification. Route routine work to an economical model and demanding work to
a more capable one.
...
## Switchyard: select models against capability limits

Businesses need to contain spend while maintaining quality across varied
requests. Use this pattern when you can describe what the economical model
handles reliably and want a judge to assess each task against those limits.
...
Semantic Router and Switchyard are alternative approaches to the same business
need. For either, measure answer quality, response time, and cost per completed
task—including routing overhead—before claiming savings.
